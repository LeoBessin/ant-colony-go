// Package parallel is v4: internal/engine/flatgrid with exactly one change.
//
// Phases A (sense) and C (decay) run on a fixed pool of worker goroutines,
// each owning a FIXED index range: worker k always senses ants
// [k*N/n, (k+1)*N/n) and always decays rows [k*H/n, (k+1)*H/n). Phase B
// (commit) stays serial on the calling goroutine, forever (CLAUDE.md §4).
// That is the ONLY change from flatgrid (CLAUDE.md §5 rule 3) — the per-ant
// arithmetic, the PRNG draws and the double buffer are copied verbatim.
//
// Why this is safe without a single lock:
//   - phase A only READS the grid; each ant's Rng and intents[i] are written
//     by the one worker owning index i;
//   - phase C reads cur/dep (neighbours may belong to another band — reads
//     only) and writes next only inside its own rows;
//   - the channel send that starts a phase and the WaitGroup that ends it are
//     the happens-before edges between phases.
//
// Why the result cannot depend on the worker count: no cell or ant is ever
// computed by more than one worker, and no value is reduced across workers.
// test/parallel_test.go checks it for several GOMAXPROCS values.
//
// Hot path this stage targets (flatgrid, large, 10 runs, 2026-09-23):
// decayGrid 75.5% cum, sense 4.8% cum, commit 10.2% cum (serial).
package parallel
...
func init() {
	simcore.Register("parallel", func() simcore.Engine { return &Engine{} })
}

// Engine is the v4 implementation. It holds no state; all state lives in World.
type Engine struct{}

func (*Engine) Name() string { return "parallel" }
